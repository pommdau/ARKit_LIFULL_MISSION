...
enum CoordinateManager {
    ...
    private static func makeRotationMatrix(angle: Float) -> simd_float2x2 {
        let rows = [
    ...
    private static func calculateRotationRadian(withCoordinates coordinates: [Coordinate]) -> Float {
        ...
        for i in coordinates.indices {
            ...
            let nextIndex = (i == coordinates.count - 1) ? 0 : i + 1
            let distance = Self.calculateDistance(from: coordinates[i], to: coordinates[nextIndex])
    ...
    private static func calculateDistance(from startPoint: Coordinate, to endPoint: Coordinate) -> Float {
        let distance = sqrt(
